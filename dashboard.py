...
app.layout = html.Div(
    ...
    children=[
        ...
        # FILTER BAR — hidden on landing via CSS, not via callback (avoids flicker)
        ...
        # Store used by CTA buttons to navigate to a tab without modifying Tabs.value directly
        dcc.Store(id="nav-store", data=None),
...
# ── CTA NAVIGATION — store-driven, avoids race condition with Tabs.value ─────
# The CTA buttons write to nav-store; a separate callback reads that store
# and updates main-tabs.value. This decouples rendering from navigation.
...
def build_landing():
    ...
    def concept_pill(term, definition, bg, fg="#ffffff"):
        return html.Span(term, title=definition, style={
    ...
    def stat_box(number, label, note, bg, num_color):
        return html.Div([
            html.Div(number, style={"fontSize":"3.2rem","fontWeight":"800",
                                    "color":num_color,"lineHeight":"1","marginBottom":"6px"}),
            html.Div(label, style={"fontSize":"0.85rem","fontWeight":"700",
                                   "color":C["gray_dark"],"lineHeight":"1.3","marginBottom":"4px"}),
            html.Div(note, style={"fontSize":"0.72rem","color":C["gray_mid"],"fontStyle":"italic"}),
        ], style={"background":bg,"borderRadius":"14px","padding":"24px 20px",
                  "textAlign":"center","boxShadow":"0 3px 14px rgba(0,0,0,0.08)","height":"100%"})
    ...
    def step_card(icon, title, desc, border_color, step_num):
        return html.Div([
            ...
            html.Div(title, style={"fontWeight":"800","fontSize":"0.9rem",
                                   "color":border_color,"marginBottom":"6px"}),
            html.Div(desc, style={"fontSize":"0.78rem","color":C["gray_mid"],"lineHeight":"1.5"}),
        ], style={"background":C["white"],"borderRadius":"12px","padding":"20px 16px",
                  "textAlign":"center","borderTop":f"4px solid {border_color}",
    ...
    def section_header(icon, title, subtitle, accent):
        return html.Div([
            ...
            html.Span(title, style={"fontSize":"1.15rem","fontWeight":"800","color":C["purple_dark"]}),
            html.Div(subtitle, style={"fontSize":"0.83rem","color":C["gray_mid"],
            ...
            dbc.Row([
                dbc.Col([
                    html.Div("Lo que no se nombra, no se atiende.", style={
                        ...
                        "color":"rgba(255,255,255,0.75)","fontStyle":"italic","marginBottom":"4px"}),
                    html.Div("Lo que no se atiende, se repite.", style={
                        "fontSize":"1.05rem","fontWeight":"600",
                        "color":"rgba(255,255,255,0.75)","fontStyle":"italic","marginBottom":"4px"}),
                    html.Div("Lo que se repite, escala.", style={
                        "fontSize":"1.1rem","fontWeight":"800",
                        "color":C["orange_light"],"fontStyle":"italic","marginBottom":"24px"}),
                    ...
                    html.P(
                        ...
                        "contra la mujer en el Perú. Este dashboard identifica dónde "
                        "está el riesgo, dónde están los servicios y dónde hay brechas urgentes.",
                        style={"fontSize":"0.92rem","color":"rgba(255,255,255,0.82)",
                               "lineHeight":"1.6","maxWidth":"600px","marginBottom":"28px"}),
                    # CTA button — id must exist in DOM for clientside_callback
                    html.Button("Explorar el dashboard →", id="cta-btn",
                        n_clicks=0,
                        style={"background":C["orange_mid"],"border":"none",
                ...
                dbc.Col([
                    html.Div([
                        ...
                        html.Div([
                            html.Div([
                                html.Span("30%", style={"fontSize":"2.8rem","fontWeight":"800","color":C["orange_light"]}),
                                html.Div("de mujeres sufrió violencia psicológica de su pareja",
                                         style={"fontSize":"0.82rem","color":"rgba(255,255,255,0.8)"}),
                            ], style={"marginBottom":"20px","paddingBottom":"20px",
                                      "borderBottom":"1px solid rgba(255,255,255,0.15)"}),
                            html.Div([
                                html.Span("5 de 7", style={"fontSize":"2.8rem","fontWeight":"800","color":C["green_light"]}),
                                html.Div("mujeres afectadas lo sufrieron en los últimos 12 meses",
                                         style={"fontSize":"0.82rem","color":"rgba(255,255,255,0.8)"}),
                            ], style={"marginBottom":"20px","paddingBottom":"20px",
                                      "borderBottom":"1px solid rgba(255,255,255,0.15)"}),
                            html.Div([
                                html.Span("70%", style={"fontSize":"2.8rem","fontWeight":"800","color":C["orange_light"]}),
                                html.Div("no busca ayuda ni denuncia lo que vive",
                                         style={"fontSize":"0.82rem","color":"rgba(255,255,255,0.8)"}),
                            ]),
    ...
    sec_problema = dbc.Container(fluid=True, style={"padding":"44px 32px 32px 32px"}, children=[
        section_header("👁️","El problema invisible",
                       "Ocurre delante de todos y nadie la llama violencia.",C["purple_mid"]),
        dbc.Row([
            dbc.Col([
                html.Div([
                    html.Div("¿Qué es la violencia psicológica?", style={
                        "fontWeight":"800","fontSize":"1rem","color":C["purple_dark"],"marginBottom":"12px"}),
                    html.P(
                        "Es toda acción u omisión que causa daño emocional, disminuye "
                        "la autoestima o perturba el desarrollo sano de la mujer. "
                        "Incluye insultos, humillaciones, control, aislamiento y amenazas.",
                        style={"fontSize":"0.87rem","color":C["gray_dark"],"lineHeight":"1.7","marginBottom":"16px"}),
                    html.P(
                        ...
                        style={"fontSize":"0.87rem","color":C["gray_dark"],"lineHeight":"1.7","marginBottom":"20px"}),
                    html.Div("Pasa el cursor sobre cada concepto para saber más 👇", style={
                        "fontSize":"0.72rem","color":C["gray_mid"],"marginBottom":"10px","fontStyle":"italic"}),
                    concept_pill("Insultos",
                        "Palabras degradantes y críticas constantes que erosionan la autoestima.",
                        C["purple_mid"]),
                    concept_pill("Humillación",
                        "Acciones que avergüenzan o degradan a la mujer frente a otros o en privado.",
                        C["purple_light"]),
                    concept_pill("Control",
                        "Vigilar con quién habla, adónde va, qué viste o cómo gasta el dinero.",
                        C["purple_dark"]),
                    ...
                    concept_pill("Amenazas",
                        ...
                        C["orange_dark"]),
                    concept_pill("Gaslighting",
                        "Negar la realidad de la víctima: 'eso no pasó', 'estás exagerando', 'estás loca'.",
                        C["gray_mid"]),
                ], style={"background":C["purple_pale"],"borderRadius":"14px","padding":"24px","height":"100%"})
            ], md=5),
            dbc.Col([
                dbc.Row([
                    dbc.Col(stat_box("30%","de mujeres sufrió violencia psicológica de su pareja",
                        "Fuente: ENDES · módulo de violencia doméstica",
                        C["orange_pale"],C["orange_dark"]), md=6, className="mb-3"),
                    dbc.Col(stat_box("5 de 7","lo vivieron en los últimos 12 meses",
                        "El problema no es solo del pasado, ocurre ahora",
                        C["green_pale"],C["green_dark"]), md=6, className="mb-3"),
                    dbc.Col(stat_box("70%","no busca ayuda institucional",
                        "Normalización, miedo, vergüenza o desconfianza",
                        C["purple_pale"],C["purple_mid"]), md=6),
                    dbc.Col(stat_box("1 de 3","casos no se denuncia nunca",
    ...
    # ── ESCALATION CHAIN ─────────────────────────────────────────────────────
    sec_escala = html.Div(style={"background":C["gray_pale"],"padding":"40px 32px"}, children=[
        dbc.Container(fluid=True, children=[
            section_header("📈","¿Por qué importa actuar ahora?",
                           "La violencia psicológica no desaparece sola — escala.",C["orange_mid"]),
            dbc.Row([
            ...
            html.Div([
                html.Span("→ ", style={"fontSize":"1.4rem","color":C["orange_mid"],"fontWeight":"800"}),
                html.Span(
                    "Identificar y atender la violencia psicológica a tiempo es la intervención "
                    "más efectiva para prevenir formas de violencia más graves. "
            ...
            ], style={"background":C["white"],"borderRadius":"12px","padding":"18px 22px",
    ...
    # ── IMPACTO + ESTADO (bridge to dashboard) ───────────────────────────────
            ...
            dbc.Row([
                dbc.Col([
                    html.Div("¿Qué hace el Estado?", style={
                        "fontSize":"1.2rem","fontWeight":"800","color":C["white"],"marginBottom":"12px"}),
                    html.P(
                        "El Estado peruano cuenta con varios mecanismos de prevención y atención. "
                        "Conocerlos, y saber dónde están y dónde faltan, es clave para actuar estratégicamente.",
                        style={"fontSize":"0.87rem","color":"rgba(255,255,255,0.8)","lineHeight":"1.7","marginBottom":"20px"}),
                    dbc.Row([
                        dbc.Col(html.Div([
                            html.Div(icon, style={"fontSize":"1.6rem","marginBottom":"6px"}),
                            html.Div(name, style={"fontWeight":"800","fontSize":"0.85rem","color":C["white"],"marginBottom":"4px"}),
                            html.Div(desc, style={"fontSize":"0.75rem","color":"rgba(255,255,255,0.7)","lineHeight":"1.5"}),
                        ], style={"background":"rgba(255,255,255,0.1)","borderRadius":"12px",
                                  "padding":"18px 14px","textAlign":"center","height":"100%",
                                  "border":"1px solid rgba(255,255,255,0.15)"}),
                        md=3, className="mb-3")
                        for icon, name, desc in [
                            ("🏢","CEM","Centros de Emergencia Mujer — atención integral en violencia"),
                            ("🏠","Hogares de Refugio","Acogida temporal para mujeres en situación de riesgo"),
                            ("📞","Línea 100","Orientación y apoyo telefónico gratuito 24/7"),
                            ("⚖️","Otros servicios","Defensorías, módulos judiciales, fiscalías especializadas"),
                        ]
                    ], className="g-3"),
                ], md=7),
                dbc.Col([
                    html.Div([
                        html.Div("🗺️ Este dashboard te permite…", style={
                            "fontWeight":"800","fontSize":"1rem","color":C["orange_light"],"marginBottom":"18px"}),
                        *[html.Div([
                            html.Span(icon + " ", style={"fontSize":"1rem"}),
                            html.Span(txt, style={"fontSize":"0.84rem","color":C["white"],"lineHeight":"1.5"}),
                        ], style={"marginBottom":"14px","display":"flex","alignItems":"flex-start","gap":"6px"})
                        for icon, txt in [
                            ("📍","Ver qué distritos tienen mayor probabilidad de violencia psicológica"),
                            ("🏥","Conocer dónde están los servicios de atención del Estado"),
                            ("⚠️","Detectar zonas con alto riesgo y baja cobertura"),
                            ("🔬","Simular el impacto de implementar nuevos servicios"),
                        ]],
                        html.Br(),
                        # CTA button 2 — also needs to exist in DOM
                        html.Button("Ir a Vista General →", id="cta-btn2",
                            n_clicks=0,
                            style={"background":C["orange_mid"],"border":"none",
                                   "borderRadius":"30px","padding":"12px 28px",
                                   "fontWeight":"700","fontSize":"0.88rem",
                                   "color":C["white"],"cursor":"pointer","marginTop":"8px"}),
