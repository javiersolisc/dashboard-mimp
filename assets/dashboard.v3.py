...
app.layout = html.Div(
    ...
    children=[
        ...
        # FILTER BAR — hidden on landing page
        html.Div(id="filter-bar-wrapper",
                 children=[dbc.Container(fluid=True, children=[make_filter_bar()])]),

        # TABS
        dbc.Container(fluid=True, style={"padding":"0 16px"}, children=[
            dcc.Tabs(id="main-tabs", value="tab-landing", children=[
                dcc.Tab(label="🏠 Inicio",                   value="tab-landing",  style=TS, selected_style=TSS),
...
# ── TOGGLE FILTER BAR (hidden on landing) ────────────────────────────────────
@app.callback(
    Output("filter-bar-wrapper","style"),
    Input("main-tabs","value"),
)
def toggle_filter_bar(tab):
    if tab == "tab-landing":
        return {"display":"none"}
...
def render_tab(tab, region, prov, area, educ, mapa_col):
    ...
    if   tab == "tab-landing":   return build_landing()
    elif tab == "tab-overview":  return build_overview(dff, mapa_col)
...
# ═══════════════════════════════════════════════════════════════
# LANDING PAGE — STORYTELLING INTRO
# ═══════════════════════════════════════════════════════════════
def build_landing():
    """
    Storytelling landing page following the narrative of the infographic:
    1. El problema invisible  →  define & name the violence
    2. La dimensión del problema  →  key stats with impact
    3. ¿Por qué escala?  →  escalation chain
    4. Conceptos clave  →  tooltips glossary
    5. ¿Qué hace el Estado?  →  bridge to dashboard
    """

    # ── helper: tooltip pill ────────────────────────────────────────────────
    def concept_pill(term, definition, bg, fg="#ffffff"):
        return html.Span([
            ...
            id={"type": "concept-pill", "term": term},
            title=definition,   # native browser tooltip — zero JS needed
            ),
        ])

    # ── helper: stat box ────────────────────────────────────────────────────
    def stat_box(number, label, note, bg, num_color):
        return html.Div([
            html.Div(number, style={
                "fontSize": "3.2rem", "fontWeight": "800",
                "color": num_color, "lineHeight": "1", "marginBottom": "6px"
            }),
            html.Div(label, style={
                "fontSize": "0.85rem", "fontWeight": "700",
                "color": C["gray_dark"], "lineHeight": "1.3", "marginBottom": "4px"
            }),
            html.Div(note, style={
                "fontSize": "0.72rem", "color": C["gray_mid"], "fontStyle": "italic"
            }),
        ], style={
            "background": bg, "borderRadius": "14px",
            "padding": "24px 20px", "textAlign": "center",
            "boxShadow": "0 3px 14px rgba(0,0,0,0.08)", "height": "100%"
        })

    # ── helper: step card (escalation) ──────────────────────────────────────
    def step_card(icon, title, desc, border_color, step_num):
        return html.Div([
            html.Div(str(step_num), style={
                "background": border_color, "color": C["white"],
                "width": "28px", "height": "28px", "borderRadius": "50%",
                "display": "flex", "alignItems": "center", "justifyContent": "center",
                "fontSize": "0.8rem", "fontWeight": "800", "marginBottom": "10px"
            }),
            html.Div(icon, style={"fontSize": "1.8rem", "marginBottom": "8px"}),
            html.Div(title, style={
                "fontWeight": "800", "fontSize": "0.9rem",
                "color": border_color, "marginBottom": "6px"
            }),
            html.Div(desc, style={
                "fontSize": "0.78rem", "color": C["gray_mid"], "lineHeight": "1.5"
            }),
        ], style={
            "background": C["white"], "borderRadius": "12px",
            "padding": "20px 16px", "textAlign": "center",
            "borderTop": f"4px solid {border_color}",
            "boxShadow": "0 2px 10px rgba(0,0,0,0.06)", "height": "100%"
        })
    ...
    def section_header(icon, title, subtitle, accent):
        return html.Div([
            ...
            html.Span(title, style={
                "fontSize": "1.15rem", "fontWeight": "800", "color": C["purple_dark"]
            }),
            html.Div(subtitle, style={
                "fontSize": "0.83rem", "color": C["gray_mid"],
                "marginTop": "2px", "borderLeft": f"3px solid {accent}",
    ...
    hero = html.Div([
        dbc.Container(fluid=True, children=[
            dbc.Row([
                ...
                dbc.Col([
                    html.Div([
                        html.Div("Lo que no se nombra, no se atiende.", style={
                            "fontSize": "1.05rem", "fontWeight": "600",
                            "color": "rgba(255,255,255,0.75)", "fontStyle": "italic",
                            "marginBottom": "4px"
                        }),
                        html.Div("Lo que no se atiende, se repite.", style={
                            "fontSize": "1.05rem", "fontWeight": "600",
                            "color": "rgba(255,255,255,0.75)", "fontStyle": "italic",
                            "marginBottom": "4px"
                        }),
                        html.Div("Lo que se repite, escala.", style={
                            "fontSize": "1.1rem", "fontWeight": "800",
                            "color": C["orange_light"], "fontStyle": "italic",
                            "marginBottom": "24px"
                        }),
                        html.H1("La herida que no se ve y que tampoco se detiene", style={
                            "fontSize": "2rem", "fontWeight": "800",
                            "color": C["white"], "lineHeight": "1.25", "marginBottom": "14px"
                        }),
                        html.P(
                            "Datos para entender y prevenir la violencia psicológica "
                            "contra la mujer en el Perú. Este dashboard es una "
                            "herramienta de política pública para identificar dónde "
                            "está el riesgo, dónde están los servicios y dónde hay "
                            "brechas urgentes.",
                            style={
                                "fontSize": "0.92rem", "color": "rgba(255,255,255,0.82)",
                                "lineHeight": "1.6", "maxWidth": "600px", "marginBottom": "28px"
                            }
                        ),
                        dbc.Button("Explorar el dashboard →", id="cta-btn",
                            n_clicks=0,
                            style={
                                "background": C["orange_mid"], "border": "none",
                ...
                # Right: quick-read stat pills
                dbc.Col([
                    html.Div([
                        html.Div("En el Perú, ahora mismo…", style={
                        ...
                        html.Div([
                            html.Div([
                                ...
                                html.Div("de mujeres sufrió violencia psicológica de su pareja", style={
                            ...
                            html.Div([
                                html.Span("5 de 7", style={
                                    "fontSize": "2.8rem", "fontWeight": "800",
                                    "color": C["green_light"]
                                }),
                                html.Div("mujeres afectadas lo sufrieron en los últimos 12 meses", style={
                                    "fontSize": "0.82rem", "color": "rgba(255,255,255,0.8)"
                                }),
                            ], style={"marginBottom": "20px",
                                      "paddingBottom": "20px",
                            ...
                            html.Div([
                                ...
                                html.Div("no busca ayuda ni denuncia lo que vive", style={
                                    "fontSize": "0.82rem", "color": "rgba(255,255,255,0.8)"
                                }),
                            ]),
                        ], style={
        ...
        "background": f"linear-gradient(135deg, {C['purple_dark']} 0%, {C['purple_mid']} 60%, {C['purple_light']} 100%)",
        "padding": "50px 32px 50px 32px",
    })

    # ══════════════════════════════════════════════════════════════════════════
    # SECTION 1 — EL PROBLEMA INVISIBLE
    # ══════════════════════════════════════════════════════════════════════════
    sec_problema = dbc.Container(fluid=True, style={"padding": "44px 32px 32px 32px"}, children=[
        section_header("👁️", "El problema invisible",
                        "La violencia psicológica ocurre delante de todos y nadie la llama violencia.",
                        C["purple_mid"]),
        dbc.Row([
            dbc.Col([
                html.Div([
                    html.Div("¿Qué es la violencia psicológica?", style={
                        "fontWeight": "800", "fontSize": "1rem",
                        "color": C["purple_dark"], "marginBottom": "12px"
                    }),
                    html.P(
                        "Es toda acción u omisión que causa daño emocional, disminuye "
                        "la autoestima, perjudica o perturba el sano desarrollo de la mujer. "
                        "Incluye insultos, humillaciones, control, aislamiento y amenazas.",
                        style={"fontSize": "0.87rem", "color": C["gray_dark"],
                               "lineHeight": "1.7", "marginBottom": "16px"}
                    ),
                    html.P(
                        "A diferencia de la violencia física, no deja marcas visibles, "
                        "lo que la hace más difícil de identificar, denunciar y atender. "
                        "Por eso, con frecuencia se normaliza y minimiza.",
                        style={"fontSize": "0.87rem", "color": C["gray_dark"],
                               "lineHeight": "1.7", "marginBottom": "20px"}
                    ),
                    html.Div([
                        html.Div("Se manifiesta como…", style={
                            "fontSize": "0.75rem", "fontWeight": "700",
                            "color": C["gray_mid"], "textTransform": "uppercase",
                            "letterSpacing": "0.06em", "marginBottom": "10px"
                        }),
                        concept_pill("Insultos",
                            "Palabras degradantes, ofensas o críticas constantes que erosionan la autoestima.",
                            C["purple_mid"]),
                        concept_pill("Humillación",
                            "Acciones que avergüenzan o degradan a la mujer frente a otros o en privado.",
                            C["purple_light"]),
                        concept_pill("Control",
                            "Vigilar con quién habla, a dónde va, qué viste o cómo gasta el dinero.",
                            C["purple_dark"]),
                        concept_pill("Aislamiento",
                            "Impedir o limitar el contacto con familiares, amigos o redes de apoyo.",
                            C["green_dark"]),
                        concept_pill("Amenazas",
                            "Intimidación mediante amenazas de daño hacia ella, sus hijos o sus seres queridos.",
                            C["orange_dark"]),
                        concept_pill("Indiferencia",
                            "Ignorar, silenciar o negar la realidad emocional de la mujer (gaslighting).",
                            C["gray_mid"]),
                    ]),
                ], style={
                    "background": C["purple_pale"], "borderRadius": "14px",
            ...
            dbc.Col([
                dbc.Row([
                    dbc.Col(stat_box("30%",
                        "de mujeres sufrió violencia psicológica de su pareja",
                        "Fuente: ENDES · módulo de violencia doméstica",
                        C["orange_pale"], C["orange_dark"]), md=6, className="mb-3"),
                    dbc.Col(stat_box("5 de 7",
                        "lo vivieron en los últimos 12 meses",
                        "El problema no es solo del pasado, ocurre ahora",
                        C["green_pale"], C["green_dark"]), md=6, className="mb-3"),
                    dbc.Col(stat_box("70%",
                        "no busca ayuda institucional",
                        "Normalización, miedo, vergüenza o desconfianza",
                        C["purple_pale"], C["purple_mid"]), md=6),
                    dbc.Col(stat_box("1 de 3",
                        "casos no se denuncia nunca",
                        "El subregistro dificulta la respuesta del Estado",
                        C["orange_pale"], C["orange_dark"]), md=6),
                ], className="g-3"),
            ], md=7),
        ], className="g-4"),
    ])
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # SECTION 2 — ¿POR QUÉ IMPORTA ACTUAR AHORA? (escalation)
    # ══════════════════════════════════════════════════════════════════════════
    sec_escala = html.Div(style={
        "background": C["gray_pale"], "padding": "40px 32px"
    }, children=[
        dbc.Container(fluid=True, children=[
            section_header("📈", "¿Por qué importa actuar ahora?",
                            "La violencia psicológica no desaparece sola — escala.",
                            C["orange_mid"]),
            dbc.Row([
                dbc.Col(step_card("🧠", "Comienza en silencio",
                    "Control, insultos, humillaciones cotidianas que parecen 'normales'.",
                    C["purple_light"], 1), md=3, className="mb-3"),
                dbc.Col(step_card("⚠️", "Se instala",
                    "Baja autoestima, ansiedad, depresión. La mujer duda de su propia percepción.",
                    C["orange_mid"], 2), md=3, className="mb-3"),
                dbc.Col(step_card("🔒", "Aísla",
                    "El agresor corta vínculos con familia y amigos. Ella pierde su red de apoyo.",
                    C["orange_dark"], 3), md=3, className="mb-3"),
                dbc.Col(step_card("🚨", "Escala",
                    "Sin intervención, puede derivar en violencia física, sexual o feminicidio.",
                    C["red_risk"], 4), md=3, className="mb-3"),
            ], className="g-3"),
            html.Div([
                html.Div("→", style={"fontSize":"1.5rem","color":C["orange_mid"],
                                     "marginRight":"10px","fontWeight":"800"}),
                html.Span(
                    "Identificar y atender la violencia psicológica a tiempo es la "
                    "intervención más efectiva para prevenir formas de violencia más graves. "
                    "Por eso este dashboard pone el foco en el riesgo territorial y la cobertura de servicios.",
                    style={"fontSize":"0.88rem","color":C["gray_dark"],"lineHeight":"1.6"}
                ),
            ], style={
                "background": C["white"], "borderRadius": "12px",
                "padding": "18px 22px", "display": "flex",
                "alignItems": "center", "marginTop": "8px",
                "borderLeft": f"4px solid {C['orange_mid']}",
                "boxShadow": "0 2px 8px rgba(0,0,0,0.05)"
            }),
        ])
    ])
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # SECTION 3 — CONSECUENCIAS (impacto en la vida)
    # ══════════════════════════════════════════════════════════════════════════
    sec_consecuencias = dbc.Container(fluid=True, style={"padding": "40px 32px"}, children=[
        section_header("💔", "Impacto en la vida de las mujeres",
                        "Las secuelas van mucho más allá del momento del maltrato.",
                        C["red_risk"]),
        dbc.Row([
            dbc.Col([
                html.Div([
                    html.Div("Consecuencias en la salud mental", style={
                    ...
                    *[
                        html.Div([
                            ...
                            html.Div([
                                html.Span(title + " — ", style={"fontWeight":"700","fontSize":"0.85rem","color":color}),
                                html.Span(desc, style={"fontSize":"0.82rem","color":C["gray_mid"]}),
                            ]),
                        ...
                        for icon, title, color, desc in [
                            ("😔","Depresión",C["purple_mid"],
                             "Tristeza profunda, pérdida de interés, dificultad para funcionar en el día a día."),
                            ("😰","Ansiedad",C["orange_dark"],
                             "Estado de alerta constante, ataques de pánico, miedo generalizado."),
                            ("🪞","Baja autoestima",C["red_risk"],
                             "Creencia internalizada de no valer, no merecer, no poder."),
                            ("🔇","Aislamiento social",C["gray_mid"],
                             "Pérdida de vínculos afectivos, soledad y dependencia del agresor."),
                            ("💤","Trastornos del sueño",C["purple_light"],
                             "Insomnio, pesadillas, hipervigilancia nocturna."),
                        ]
                    ],
                ])
            ], md=6),
            dbc.Col([
                html.Div([
                    html.Div("¿Quiénes tienen mayor riesgo?", style={
                        "fontWeight": "700", "fontSize": "0.82rem",
                        "color": C["gray_mid"], "textTransform": "uppercase",
                        "letterSpacing": "0.05em", "marginBottom": "12px"
                    }),
                    *[
                        html.Div([
                            html.Div(pct, style={
                                ...
                                "color": col, "minWidth": "65px", "textAlign": "center"
                            }),
                            html.Div([
                                html.Div(label, style={"fontWeight":"700","fontSize":"0.85rem",
                                                       "color":C["gray_dark"]}),
                                html.Div(sub, style={"fontSize":"0.77rem","color":C["gray_mid"],"marginTop":"2px"}),
                            ]),
                        ], style={"display":"flex","alignItems":"center","gap":"16px",
                                  "padding":"12px 16px","marginBottom":"10px",
                                  "background":C["white"],"borderRadius":"10px",
                                  "boxShadow":"0 1px 6px rgba(0,0,0,0.05)",
                                  "borderRight":f"3px solid {col}"})
                        for pct, label, sub, col in [
                            ("+40%","Mujeres en zonas rurales",
                             "Menor acceso a servicios y mayor aislamiento geográfico.",C["orange_dark"]),
                            ("+35%","Sin educación secundaria completa",
                             "Mayor dependencia económica y menor acceso a información.",C["red_risk"]),
                            ("+30%","En situación de pobreza",
                             "Barreras económicas para salir de la relación o buscar ayuda.",C["purple_mid"]),
                            ("+25%","Entre 18 y 35 años",
                             "Grupo de mayor exposición en relaciones de pareja activas.",C["green_dark"]),
                        ]
                    ],
                    html.Div([
                        html.Span("ℹ️ ", style={"fontSize":"0.9rem"}),
                        html.Span(
                            "Datos basados en ENDES y literatura especializada. "
                            "El dashboard permite explorar estos perfiles de riesgo por territorio.",
                            style={"fontSize":"0.73rem","color":C["gray_mid"],"fontStyle":"italic"}
                        ),
                    ], style={"marginTop":"12px","padding":"8px 12px",
                              "background":C["gray_pale"],"borderRadius":"8px"})
                ])
            ], md=6),
        ], className="g-4"),
    ])
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # SECTION 4 — ¿QUÉ HACE EL ESTADO? (bridge to dashboard)
    # ══════════════════════════════════════════════════════════════════════════
    sec_estado = html.Div(style={
        "background": f"linear-gradient(135deg, {C['purple_dark']} 0%, {C['purple_mid']} 100%)",
        "padding": "44px 32px"
    }, children=[
        dbc.Container(fluid=True, children=[
            dbc.Row([
                dbc.Col([
                    html.Div("¿Qué hace el Estado?", style={
                        "fontSize": "1.2rem", "fontWeight": "800",
                        "color": C["white"], "marginBottom": "12px"
                    }),
                    html.P(
                        "El Estado peruano cuenta con varios mecanismos de prevención "
                        "y atención. Conocerlos, y saber dónde están y dónde faltan, "
                        "es clave para actuar de forma estratégica.",
                        style={"fontSize":"0.87rem","color":"rgba(255,255,255,0.8)",
                               "lineHeight":"1.7","marginBottom":"20px"}
                    ),
                    dbc.Row([
                        *[dbc.Col(html.Div([
                            html.Div(icon, style={"fontSize":"1.6rem","marginBottom":"6px"}),
                            html.Div(name, style={"fontWeight":"800","fontSize":"0.85rem",
                                                  "color":C["white"],"marginBottom":"4px"}),
                            html.Div(desc, style={"fontSize":"0.75rem",
                                                  "color":"rgba(255,255,255,0.7)","lineHeight":"1.5"}),
                        ], style={
                            "background":"rgba(255,255,255,0.1)","borderRadius":"12px",
                            "padding":"18px 14px","textAlign":"center","height":"100%",
                            "border":"1px solid rgba(255,255,255,0.15)"
                        }), md=3, className="mb-3")
                        for icon, name, desc in [
                            ("🏢","CEM",
                             "Centros de Emergencia Mujer — atención integral en violencia"),
                            ("🏠","Hogares de Refugio",
                             "Acogida temporal para mujeres en situación de riesgo"),
                            ("📞","Línea 100",
                             "Orientación y apoyo telefónico gratuito 24/7"),
                            ("⚖️","Otros servicios",
                             "Defensorías, módulos judiciales, fiscalías especializadas"),
                        ]],
                    ], className="g-3"),
                ], md=7),
                dbc.Col([
                    html.Div([
                        html.Div("🗺️ Este dashboard te permite…", style={
                            "fontWeight": "800", "fontSize": "1rem",
                            "color": C["orange_light"], "marginBottom": "18px"
                        }),
                        *[html.Div([
                            html.Span(icon + " ", style={"fontSize":"1rem"}),
                            html.Span(txt, style={"fontSize":"0.84rem","color":C["white"],
                                                  "lineHeight":"1.5"}),
                        ], style={"marginBottom":"14px","display":"flex","alignItems":"flex-start","gap":"6px"})
                        for icon, txt in [
                            ("📍","Identificar los distritos con mayor probabilidad de violencia psicológica"),
                            ("🏥","Ver dónde están los servicios de atención del Estado"),
                            ("⚠️","Detectar brechas: zonas con alto riesgo y baja cobertura"),
                            ("🔬","Simular el impacto de implementar nuevos servicios"),
                            ("📊","Explorar perfiles de riesgo por características individuales"),
                        ]],
                        html.Div(style={"height":"16px"}),
                        dbc.Button("Ir a Vista General →", id="cta-btn2",
                            n_clicks=0,
                            style={
                                "background":C["orange_mid"],"border":"none",
    ...
    return html.Div([
        hero,
        sec_problema,
        html.Hr(style={"margin":"0","border":f"1px solid {C['gray_pale']}"}),
        sec_escala,
        html.Hr(style={"margin":"0","border":f"1px solid {C['gray_pale']}"}),
        sec_consecuencias,
        sec_estado,
...
@app.callback(
    ...
    Input("cta-btn","n_clicks"),
    Input("cta-btn2","n_clicks"),
    prevent_initial_call=True,
)
def go_to_overview(n1, n2):
    return "tab-overview"
